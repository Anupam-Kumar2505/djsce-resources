  ...
  upload.array("files", 10)(req, res, (err) => {
    if (err instanceof multer.MulterError) {
      ...
      if (err.code === 'LIMIT_FILE_SIZE') {
        return res.status(400).json({ error: "File too large. Maximum size is 10MB per file due to Cloudinary free plan limits." });
      }
    ...
    for (let i = 0; i < req.files.length; i++) {
      ...
      try {
        ...
        if (!uploadResult) {
          ...
          errors.push(`Failed to upload ${file.originalname} to cloud storage`);
          continue;
        }
